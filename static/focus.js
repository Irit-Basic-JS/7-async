...
async function run() {
    let orgs = await sendRequestPromise(API.organizationList);
    let orgsRegNums = orgs.join(',');

    await Promise.all([
        sendRequestPromise(`${API.orgReqs}?ogrn=${orgsRegNums}`),
        sendRequestPromise(`${API.analytics}?ogrn=${orgsRegNums}`),
        sendRequestPromise(`${API.buhForms}?ogrn=${orgsRegNums}`)
